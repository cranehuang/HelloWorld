...
//    }
    ...
    private String buildTimestampLine(String timestamp) {
        if (!mMultiRecipients || mMessageItem.isOutgoingMessage() || TextUtils.isEmpty(mMessageItem.mContact)) {
...
//    }
	...
	private void updateAvatarView(String addr, boolean isSelf) {
        ...
        if (isSelf || !TextUtils.isEmpty(addr)) {
            Contact contact = isSelf ? Contact.get("", false) : Contact.get(addr, false);
    ...
    private void bindCommonMessage(final boolean sameItem) {
        ...
        boolean haveLoadedPdu = mMessageItem.isSms();
        ...
        if (!sameItem || haveLoadedPdu) {
            boolean isSelf = MessageUtils.isOutgoingFolder(mMessageItem.mBoxId);
